...
def test_order_pair_by_mendeleev_and_label():
    # U = 20    Rh = 59    In = 75
    expected = pair_order.order_pair_by_mendeleev(("In", "U"))
    assert expected == ("U", "In")

    expected = pair_order.order_pair_by_mendeleev(("U", "In"))
    assert expected == ("U", "In")

    expected = pair_order.order_pair_by_mendeleev(("Rh", "U"))
    ...
    expected = pair_order.order_pair_by_mendeleev(("Co2B", "Co2A"))
    assert expected == ("Co2A", "Co2B")

    expected = pair_order.order_pair_by_mendeleev(("Co2A", "Co2B"))
    assert expected == ("Co2A", "Co2B")
