# Use this script to plot from JSON

# Identify folder containing

# List all the folders

# Check whether the folder contains .json files

# Use the Histogram to replot with varying range (or use the default) and the bin width (or use the default)

# Set up the range, set up the bin width
...
def plot_histogram():
    ...
    selected_dirs = prompt.get_user_input_folder_processing(dir_names_with_json, ".json")
    ...
    for idx, dir_name in enumerate(selected_dirs.values(), start=1):
        ...
        if (
            ...
            if not is_custom_design:
                histogram.draw_histograms(site_pair_dict, element_pair_dict, histogram_output_dir)
