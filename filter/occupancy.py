...
def get_coord_occupancy_sum(cif_loop_values):
    ...
    for i in range(num_atom_labels):
        _, occupancy, coordinates = (
            cif_parser.get_atom_info(cif_loop_values, i)
        )
        occupancy_num = (
            coord_occupancy_sum.get(coordinates, 0)
            + occupancy
        )
...
def get_atom_site_mixing_info(cif_loop_values) -> str:
    ...
    coord_occupancy_sum = get_coord_occupancy_sum(
        cif_loop_values
    )
    ...
    is_atomic_mixing = (
        len(coord_occupancy_sum) != num_atom_labels
    )
...
def get_all_possible_ordered_label_pairs(cif_loop_values):
    ...
    label_list = cif_parser.get_atom_label_list(
        cif_loop_values
    )
    all_possible_label_pairs = list(
        product(label_list, repeat=2)
    )
...
def get_atom_site_mixing_dict(
    ...
    cif_loop_value_dict = (
        cif_parser.get_cif_loop_value_dict(cif_loop_values)
    )
    ...
    if atom_site_mixing_file_info != "4":
        for pair in unique_ordered_label_pairs:
            ...
            first_label_coord = cif_loop_value_dict[
                pair[0]
            ]["coordinates"]
            second_label_coord = cif_loop_value_dict[
                pair[1]
            ]["coordinates"]
            ...
            if (
                occupancy_sum[first_label_coord]
                - first_label_occ
            ) == 0:
            ...
            if (
                occupancy_sum[second_label_coord]
                - second_label_occ
            ) == 0:
