...
def preprocess_cif_file_on_label_element(file_path):
    ...
    for line in content_lines:
        ...
        atom_type_from_label = cif_parser.get_atom_type(
            site_label
        )
        ...
        if "," in site_label:
            ...
            # Get 'Er1In3B'
            site_label = site_label.replace(",", "").split(
                " "
            )[0]

            # Get ['Er', 'Co', 'In']
            elements = re.findall(
                "[A-Z][a-z]*", compound_formula
            )
            ...
            line = line.replace(
                site_label_original, modified_label
            )
    ...
    if is_cif_file_updated:
        ...
        (
            start_index,
            end_index,
        ) = cif_parser.get_line_start_end_line_indexes(
            file_path, "_atom_site_occupancy"
        )
