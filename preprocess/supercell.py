...
def calculate_distance(point1, point2, cell_lengths, angles):
    ...
    cross_x = 2 * cell_lengths[1] * cell_lengths[2] * np.cos(angles[0]) * delta_y * delta_z
    cross_y = 2 * cell_lengths[2] * cell_lengths[0] * np.cos(angles[1]) * delta_z * delta_x
    cross_z = 2 * cell_lengths[0] * cell_lengths[1] * np.cos(angles[2]) * delta_x * delta_y
...
def get_coords_after_sym_operations(block,
                                    atom_site_fract_x,
                                    atom_site_fract_y,
                                    atom_site_fract_z,
                                    atom_site_label):
...
def get_points_and_labels(all_coords_list,
                          loop_values,
                          supercell_generation_method):
    """
