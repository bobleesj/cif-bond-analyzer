...
def get_cif_info(file_path, loop_tags, supercell_generation_method=3):
    ...
    return cif_block, cell_lengths, cell_angles_rad, \
           all_coords_list, all_points, unique_labels, atom_site_list
...
def get_folder_and_files_info(
    ...
    files_lst = [os.path.join(folder_info, file)
                 for file in os.listdir(folder_info) if file.endswith('.cif')]
