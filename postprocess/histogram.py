...
def plot_histograms(data, directory_path, output_filename):
    ...
    for image_num in range(total_images):
        ...
        fig, axes = plt.subplots(num_rows, max_columns, figsize=(max_columns * 4, num_rows * 3))
        axes = axes.flatten()
        ...
        for i in range(histograms_per_image):
            if i < len(current_pairs):
                ...
                for cat, color in categories_colors.items():
                    category_distances = [float(info["dist"]) for infos in records.values() for info in infos if info["mixing"] == cat]
                    if category_distances:
                        stacked_data.append(category_distances)
                        labels.append(cat)
                if stacked_data:
                    ax.hist(stacked_data, bins=bins, color=[categories_colors[cat] for cat in labels], label=[categories_mapping[cat] for cat in labels], stacked=True, edgecolor="black")
        ...
        fig.savefig(os.path.join(output_dir, f"{output_filename}_{image_num + 1}.png"), dpi=150)
        plt.close(fig)
