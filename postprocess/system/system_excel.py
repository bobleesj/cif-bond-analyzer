...
def save_structure_analysis_excel(
    ...
    for structure, info in structure_dict.items():
        ...
        first_row = True
        for bond_type, bond_info in info[
            "bond_data"
        ].items():
            row = [
    ...
    output_file_path = join(
        output_dir, "system_analysis_main.xlsx"
    )
...
def save_bond_overview_excel(
    ...
    bond_types = [
        f"{pair[0]}-{pair[1]}"
        for pair in possible_bond_pairs
    ]

    # Initialize structure bond counts
    unique_structure_bond_counts = {
    ...
    output_file_path = join(
        output_dir, "system_analysis_files.xlsx"
    )
    df.to_excel(output_file_path, index=False)
    print(df.head(20))
