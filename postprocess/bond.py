...
def get_atom_site_labeled_dict(
    ...
    atom_site_dict_postprocessed = (
        postprocess_atom_site_dict(
...
def postprocess_atom_site_dict(
    ...
    for site1, site2, min_dist in pairs_list:
        ...
        if (
            filename
            not in atom_site_dict_processed[pair_label]
        ):
            atom_site_dict_processed[pair_label][
                filename
            ] = []
...
def get_shortest_distance(values):
    ...
    for value in values:
        ...
        if (
            shortest_dist is None
            or distance < shortest_dist
        ):
...
def get_atom_site_dict_with_no_number(input_dict):
    ...
    for pair_key, values in input_dict.items():
        element_pair_key = get_element_pair_from_label_pair(
            pair_key
        )
        ...
        for id, id_values in values.items():
            ...
            for value in id_values:
                if (
                    value
                    not in output_dict[element_pair_key][id]
                ):
                    output_dict[element_pair_key][
                        id
                    ].append(value)
    return output_dict
...
def get_element_dict(input_dict):
    ...
    for pair_key, values in input_dict.items():
        ...
        output_dict.setdefault(element_pair_key, {})
        for id, id_values in values.items():
            shortest_dist, shortest_mixing = (
                get_shortest_distance(id_values)
            )
            output_dict[element_pair_key][id] = [
                {
                    "dist": str(shortest_dist),
                    "mixing": str(shortest_mixing),
                }
            ]
    return output_dict
...
def append_atom_site_dict(
    ...
    for pair_key, values in atom_site_pair_dict.items():
        ...
        for id, id_values in values.items():
            if (
                id
                not in global_atom_site_pair_dict[pair_key]
            ):
                global_atom_site_pair_dict[pair_key][
                    id
                ] = []
            ...
            for value in id_values:
                if (
                    value
                    not in global_atom_site_pair_dict[
                        pair_key
                    ][id]
                ):
                    global_atom_site_pair_dict[pair_key][
    ...
    for pair_key, values in atom_site_pair_dict.items():
        ...
        for id, id_values in values.items():
            ...
            for value in id_values:
                if (
                    value
                    not in global_element_pair_dict[
                        pair_key
                    ][id]
                ):
                    global_element_pair_dict[pair_key][
