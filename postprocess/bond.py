...
def process_and_order_pairs(all_points, atomic_pair_list):
    ...
    for i in range(len(all_points)):
        ...
        if min_dist_pair is not None:
            processed_pairs.append(min_dist_pair)
...
def get_unique_pairs_dict(ordered_pairs, filename):
    """
    Constructs a dictionary containing the shortest distance pairs
    for each unique atom label pair per file. It iterates over a
    list of ordered pairs, where each pair contains atom labels and
    the distance between those atoms. If a pair of labels is encountered
    for the first time or if the current pair's distance is shorter
    than the previously recorded distance for the same label pair,
    it updates the dictionary to include the current pair.
    """
    unique_pairs_dict = {}
...
def get_dist_mix_element_pair_dict(input_dict):
    """
    Creates element pair dict containing filenames, mixing, distances.
    """
    output_dict = {}
