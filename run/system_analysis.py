...
def conduct_system_analysis(dir_path):
    ...
    # Read the JSON file
    updated_json_file_path = get_updated_json_file_path(
        dir_path
    )
    ...
    (
        ...
        unique_formulas,
    ) = system_util.parse_data_from_json_and_file(
        bond_data, dir_path
    )
    system_util.write_json_data(
    ...
    is_single_binary = system_util.get_is_single_binary(
        updated_json_file_path
    )
    is_double_binary = system_util.get_is_double_binary(
        updated_json_file_path
    )
    is_ternary = system_util.get_is_ternary(
        updated_json_file_path
    )
...
def get_updated_json_file_path(dir_path):
    folder_name = os.path.basename(dir_path)
    updated_json_file_path = f"{dir_path}/output/updated_{folder_name}_site_pairs.json"
